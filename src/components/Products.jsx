    ...
    <>
      <div className="p-6">
        <div className="flex justify-between items-center mb-4">
          ...
          <Link to="/AddItems" className="bg-green-500 text-white px-4 py-2 rounded shadow-md hover:bg-green-600">
            Add Item
          </Link>
        </div>
            ...
            <tbody className="text-gray-600 text-sm font-light">
              {products && Array.isArray(products) && products.map((row, index) => (
                <tr key={index} className="border-b border-gray-200 hover:bg-gray-100">
                  <td className="">
                    <img src={row.picture} className='py-3 px-4 h-24 w-24 rounded-full text-left' alt="product"/>
                  </td>
